Preview — copy the markdown below

James Fairbairn
Principal Engineer · MLOps · Cloud DevOps · AI Agent Systems

Principal Engineer with 18 years across finance, defence, broadcast, and the public sector. Currently focused on AI agent systems, MCP server development, and MLOps infrastructure. Based in Edinburgh, Scotland. Available for contract roles.

### What I'm building

* **OANDA MCP Server** — MCP server wrapping the OANDA V20 REST API. Lets AI agents query market data, manage orders, and control positions. Built with FastMCP, supports stdio and HTTP transport.
* **RAG pipelines** — Local and LangChain-based RAG with ChromaDB, Ollama, Streamlit, and LangSmith tracing.
* **MLOps platform** — KServe on EKS/minikube with ArgoCD GitOps, GPU node groups, and full model hosting infrastructure.

---

### Focus areas

`AI agent systems` `MCP development` `DevOps` `MLOps` `KServe / EKS` `AWS CDK / Terraform` `Python / FastAPI` `CI/CD`

---

### Get in touch

[LinkedIn](https://www.linkedin.com/in/james-fairbairn-5301494) · james.fairbairn@outlook.com
